...
class MetaNN:
	def __init__(self, input_shape):
		with tf.device('/gpu:0'):
			...
			x = Conv2D(32, kernel_size = 3, strides = (1,1),  activation = 'relu')(x) # 17-3+1=15
			x = MaxPooling2D(pool_size = (2,2), padding='same')(x) #8
			x1 = Dropout(0.5)(x)

			x = Conv2D(64, (3,3), padding='same', activation = 'relu')(x1)
			x = Conv2D(64,(3,3), padding='same', activation = 'relu')(x) #8x8
			x = MaxPooling2D(pool_size = (2,2), padding='same')(x) #4
			x = Dropout(0.5)(x) #4x4

                        # add shortcut connection
                        #x = x + Conv2D(64, kernel_size = 1, strides=(2,2), padding='same')(x1)
...
class Agent(Visualizable):
	...
	def identify_optimal_macro_action(self, agent_host, starting_loc):
		if agent_host._world[starting_loc[0], starting_loc[1]] == 'x':
			...
			for action in range(4):
				direction = possible_actions[action]
				state = (starting_loc[0]+direction[0], starting_loc[1]+direction[1])
				if state[0]>=0 and state[0]<=16 and state[1]>=0 and state[1]<=16 and agent_host._world[state[0], state[1]] == 'w':
					if self.expert_value[state] > max_value:
						max_value = self.expert_value[state]
						best_direction = direction

...
class Hdqn:
    ...
    def saveWeight(self, model_dir, subgoal):
        #self.controllerNet.save_weights(recordFolder+'/policy_subgoal_' + str(subgoal) + '.h5')
        path =  os.path.join(model_dir, 'netSubgoal_' + str(subgoal) + '.h5')
        self.controllerNet.save_weights(path)

    def loadWeight(self, model_dir, subgoal):
        #path = 'weight/'
        #self.controllerNet.load_weights('standardized_models/policy_subgoal_' + str(subgoal) + '.h5')
        path =  os.path.join(model_dir, 'netSubgoal_' + str(subgoal) + '.h5')
        self.controllerNet.load_weights(path)
        self.controllerNet.reset_states()

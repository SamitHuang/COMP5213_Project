...
class MetaNN:
    def __init__(self, model_dir):
        ...
        with tf.device('/gpu:'+str(GPU)):
            ...
            self.meta_controller.save_weights(model_dir+"/initial_random_weights_metacontroller.h5")
    ...
    def reset(self):
        rmsProp = optimizers.RMSprop(lr=0.00025, rho=0.95, epsilon=1e-08, decay=0.0)
        self.meta_controller.load_weights(self.model_dir+"/initial_random_weights_metacontroller.h5")
        self.meta_controller.compile(loss = 'categorical_crossentropy', optimizer = rmsProp)
    ...
    def train(self):
        # TODO: something goes wrong here causing thread killed when training one subgoal for too long.
        ...
        try: # TODO: why try? nothing risky. what's diff between _num_valid and BATCH_SIZE
            self._samples = range(self._num_valid)
            BATCH_SIZE = len(self._samples)
        except:
            print("Exception comes out: BATCH_SIZE and _num_valid", BATCH_SIZE, self._num_valid)
    ...
    def saveWeight(self, model_dir):
        path =  os.path.join(model_dir, 'metaController.h5')
        self.meta_controller.save_weights(path)

    def loadWeight(self, model_dir):
        path =  os.path.join(model_dir, 'metaController.h5')
        self.meta_controller.load_weights(path)
